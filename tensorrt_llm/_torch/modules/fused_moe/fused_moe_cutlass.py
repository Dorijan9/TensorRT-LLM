...
class CutlassFusedMoE(MoE):
    ...
    def _get_quant_method(self):
        ...
        # INT8 weight-only kernel. Supports both gated (Swiglu) and
        # non-gated (Relu2 — Nemotron-H) experts. Requires the
        # companion moeOp.cpp validation fix (v18) for non-gated.
        ...
        if w8a16_on_the_fly_enabled() and unquantized_config:
            logger.info_once(
                "CutlassFusedMoE: TRTLLM_MOE_W8A16_ONTHEFLY=1 active (v18); "
                "quantizing routed MoE expert weights to INT8 at load time",
                key="w8a16_onthefly_v18_active",
            )
